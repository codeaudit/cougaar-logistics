...
module Cougaar
  module Actions
    ...
    class DynamicSD < Cougaar::Action
      ...
      def initialize(run, agent, role, availability, start_date, end_date, autopublish = true)
        super(run)
          @role = role
          ...
          @agent = agent
          @add = "Add"
          @publish = "Publish+All+Rows" if autopublish
          @protocol = "http"
      end

      def perform
        cougaar_agent = @run.society.agents[@agent]
        @run.info_message "DynamicSD for agent #{@agent}[#{cougaar_agent}]...role:#{@role}, avail:#{@availability}, sd #{@startdate}, ed:#{@enddate}"
        if cougaar_agent
