 ...
 * **********************************************************************
 * 
 * $Source: /opt/rep/cougaar/logistics/bbn/datagrabber/src/org/cougaar/mlm/ui/newtpfdd/gui/view/Attic/EarthImagePlugIn.java,v $
 * $Revision: 1.1 $
 * $Date: 2002-05-14 20:41:06 $
 * $Author: gvidaver $
 * 
 * **********************************************************************
 */
...
public class EarthImagePlugIn extends AbstractPlugIn implements ImageServerConstants {
	...
	// The first time through with a good ii, the it will be
	...
	try {
	    ...
	    if (Debug.debugging("earthimage") && ii != null) {
		Debug.output("EarthImagePlugIn: ii OK");
	    }
    ...
    /**
     * The ImageTranslator is the object that takes the ImageIcon and
	...
	/** Image Icon height, */
	public int iheight;
	/** Horizontal degrees/pixel in the source ImageIcon.  Assumed
	 *  to be constant across the image. */
	public float hor_dpp;
	/** Vertical degrees/pixel in the source ImageIcon.  Assumed
	 *  to be constant across the image. */
	 ...
	 *  image.
	 */
	public ImageTranslator(ImageIcon ii) {
	    if (ii != null) {
		iwidth = ii.getIconWidth();
		iheight = ii.getIconHeight();
		...
		pixels = getPixels(ii.getImage(), 0, 0, iwidth, iheight);
...
//  			    Debug.message("earthimage", "ImageTranslator: outside pixel range");
	...
	/**
	 * Get the pixels from the ImageIcon Image.  If anything goes
